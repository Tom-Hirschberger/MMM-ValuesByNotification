...
| unitSpace | If you want to have a small space added before the unit (between value and unit usually) and do not want to solve it with CSS you can set this option to `true`. | Boolean | false |
...
The options "valueUnit", "naValue", "valueTitle", "valueIcon", "valueImgIcon", "valueFormat", "unitSpace", "formatNaValue", "jsonpath", "newlineReplacement", "valuePositions", "thresholds" and "automaticWrapperClassPrefix" can be configured within the items or groups or general configuration as well. If multiple options are configured the deepest hirarchical option overrides the other ones.
